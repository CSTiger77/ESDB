...
from lib.utils import get_start_end_index
...
class ExemplarManager:
    ...
    def construct_exemplar_set(self, dataset, model, to_store_number, batch_size, num_workers, active_classes_num=None,
                               hard_rate=None, split_block_nums=None, **kwargs):
        ...
        elif "kmeans" in self.mng_approach:  # kmeans
            exemplar_set = self.kmeans_selector(dataset, model, batch_size, num_workers, to_store_number, **kwargs)
        elif "confidence" in self.mng_approach:
            exemplar_set = self.confidence_selector(dataset, model, batch_size, num_workers, to_store_number,
                                                    active_classes_num,
                                                    hard_rate,
                                                    **kwargs)  # todo

        elif "rainbow" in self.mng_approach:
            exemplar_set = self.rainbow_selector(dataset, model, batch_size, num_workers, to_store_number,
                                                 active_classes_num,
                                                 split_block_nums,
                                                 **kwargs)  # todo
            pass
    ...
    @staticmethod
    def _get_extracted_features(model, image_batch, **kwargs):
        features = model(x=image_batch, is_nograd=True, feature_flag=True)
        if "ensemble_handling" in kwargs:
            if isinstance(features, list):
                if kwargs["ensemble_handling"] == 'last':
                    features = features[-1]
                elif kwargs["ensemble_handling"] == 'all':
                    features = torch.cat(features, dim=1)
                else:
                    raise NotImplementedError(f'Unknown handdling of multiple features {kwargs["ensemble_handling"]}')
            elif len(features.shape) == 3:  # joint tokens
                if kwargs["ensemble_handling"] == 'last':
                    features = features[-1]
                elif kwargs["ensemble_handling"] == 'all':
                    features = features.permute(1, 0, 2).view(len(image_batch), -1)
                else:
                    raise NotImplementedError(f'Unknown handdling of multiple features {kwargs["ensemble_handling"]}')
        return features
    ...
    def herding(self, dataset, model, batch_size, num_workers, to_store_number, **kwargs):
        ...
        for (image_batch, _, _) in dataloader:
            image_batch = image_batch.to(self._device)
            feature_batch = self._get_extracted_features(model, image_batch, **kwargs).cpu()
            if first_entry:
                features = feature_batch
                first_entry = False
            else:
        ...
        if "herding" in self._centroid_order:
            ...
            if self.store_original_imgs:
                ...
                exemplar_set = [dataset[centroid_idex][0].numpy() for centroid_idex in
                                cluster_label_2_feature_centroid_index]
            random.shuffle(exemplar_set)
        else:
            raise RuntimeError('self._centroid_order is illegal: {}'.format(self._centroid_order))
        return exemplar_set
        pass

    def confidence_selector(self, dataset, model, batch_size, num_workers, to_store_number, active_classes_num=None,
                            hard_rate=None, **kwargs):
        exemplar_set = []
        assert active_classes_num is not None
        print(f"Use confidence_selector to construct exemplar sets")
        first_entry = True
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
                                pin_memory=False, drop_last=False)
        # selected_indexs = torch.tensor([-1 for i in range(to_store_number)]).to(self._device)
        confidences = None
        negative_nums = int(0)
        for (transformed_image_batch, label_batch, _) in dataloader:
            transformed_image_batch = transformed_image_batch.to(self._device)
            label_batch = label_batch.to(self._device)
            confidence_batch, negative_nums_temp = self._get_softmax_confidence(model, transformed_image_batch,
                                                                                label_batch, active_classes_num)  # todo
            if first_entry:
                confidences = confidence_batch
                first_entry = False
            else:
                confidences = torch.cat([confidences, confidence_batch], dim=0)
            negative_nums += negative_nums_temp
        sorted_confidences, confidence_indexs = torch.sort(confidences, descending=True)
        if hard_rate is not None:
            assert 0 <= hard_rate <= 1.
            hard_nums = int(hard_rate * to_store_number)
            easy_nums = int(to_store_number - hard_nums)
            assert 0 <= easy_nums <= to_store_number
            if easy_nums > 0:
                if easy_nums < to_store_number:
                    selected_indexs = confidence_indexs[0:easy_nums]
                    if negative_nums > 0.5:
                        selected_indexs = torch.cat([selected_indexs,
                                                     confidence_indexs[-(hard_nums + negative_nums):-negative_nums]],
                                                    dim=0)
                    else:
                        selected_indexs = torch.cat([selected_indexs,
                                                     confidence_indexs[-hard_nums:]],
                                                    dim=0)
                else:
                    selected_indexs = confidence_indexs[0:to_store_number]
            else:
                assert hard_nums == to_store_number
                if negative_nums > 0.5:
                    selected_indexs = confidence_indexs[-(hard_nums + negative_nums):-negative_nums]
                else:
                    selected_indexs = confidence_indexs[-hard_nums:]
            pass
        else:
            selected_indexs = confidence_indexs[0:to_store_number]
        assert selected_indexs.size(0) == to_store_number
        # print("confidences:", confidences)
        # print("selected_indexs:", len(selected_indexs), selected_indexs)
        assert -math.inf not in confidences[selected_indexs]
        if self.store_original_imgs:
            exemplar_set = [dataset[idex][-1] for idex in
                            selected_indexs]
        else:
            exemplar_set = [dataset[idex][0].numpy() for idex in
                            selected_indexs]
        random.shuffle(exemplar_set)
        return exemplar_set
        pass

    def rainbow_selector(self, dataset, model, batch_size, num_workers, to_store_number, active_classes_num=None,
                         split_block_nums=None, **kwargs):
        exemplar_set = []
        assert active_classes_num is not None
        print(f"Use confidence_selector to construct exemplar sets")
        first_entry = True
        ...
        for (transformed_image_batch, label_batch, _) in dataloader:
            ...
            negative_nums += negative_nums_temp
        sorted_confidences, confidence_indexs = torch.sort(confidences, descending=True)
        positive_nums = confidences.size(0) - negative_nums
        start_indexs, end_indexs = get_start_end_index(positive_nums, to_store_number, split_block_nums)
        for i in range(split_block_nums):
            if i == 0:
                selected_indexs = confidence_indexs[start_indexs[i]:end_indexs[i]]
            else:
                selected_indexs = torch.cat([selected_indexs, confidence_indexs[start_indexs[i]:end_indexs[i]]],
                                            dim=0)
        assert selected_indexs.size(0) == to_store_number
        if self.store_original_imgs:
            exemplar_set = [dataset[idex][-1] for idex in
    ...
    def NCM_classify(self, x, model, allowed_classes=None, **kwargs):
        ...
        if self.compute_means:
            ...
            for class_id in range(len(self.exemplar_sets)):
                ...
                features = self._get_extracted_features(model, exemplars, **kwargs)
                if self._norm_exemplars:
                    features = F.normalize(features, p=2, dim=1)
        ...
        feature = self._get_extracted_features(model, x, **kwargs)  # (batch_size, feature_size)
        if self._norm_exemplars:
    ...
    def _get_softmax_confidence(self, model, transformed_image_batch, lable_batch, active_classes_num):
        out = model(x=transformed_image_batch, is_nograd=True, get_classifier=True)
        logit_batch, fc_y_hat = torch.max(out[:, 0:active_classes_num], 1)
        a = fc_y_hat.eq(lable_batch.data)
        b = ~a
        logit_batch[b] = -math.inf
        negative_nums = b.sum()
        return logit_batch, negative_nums
        pass

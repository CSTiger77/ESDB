...
from lib.model.loss import CrossEntropy_binary, CrossEntropy, loss_fn_kd, BalancedSoftmax, dive_loss, BKD
...
from lib.utils.utils import get_optimizer, get_scheduler, skew_pre_model_output_for_distill, \
    construct_sample_num_per_class, construct_LT_dataset, construct_label_weight, \
    construct_effective_weight_per_class, construct_LT_train_dataset_split


class skewKD_LT_KD_handler:
    """Our approach DDC"""

    def __init__(self, dataset_handler, cfg, logger, device):
    ...
    def task_init(self):
        ...
        '''LT_classes_split=[50, 50], LT_classes_sample_num=[20, 500]'''
        self.sample_num_per_class = construct_sample_num_per_class(self.dataset_handler.all_classes,
                                                                   self.cfg.DATASET.LT_classes_split,
                                                                   self.cfg.DATASET.LT_classes_sample_num)
        ...
        self.logger.info(f"self.LT_val_dataset length: {len(self.LT_train_dataset)}")
    ...
    def skewKD_LT_KD_train_main(self):
        ...
        if self.cfg.model.oversample:
            self.logger.info(
                "Run skewKD_LT_KD_train_oversample."
            )
            self.skewKD_LT_KD_train_oversample(model_dir=model_dir, label_weight=label_weight,
                                               model_per_class_weights=model_per_class_weights)
        else:
            self.skewKD_LT_KD_train(model_dir=model_dir, label_weight=label_weight,
                                    model_per_class_weights=model_per_class_weights)
    ...
    def skewKD_LT_train_main(self):
        ...
        if "cRT" in self.cfg.approach:
            ...
            if self.cfg.teacher.teacher_model_path:
                self.teacher_model = resnet_model(self.cfg)
        ...
        # else:
        #     raise ValueError(f"Wrong approach {self.cfg.approach}")
            ...
            while iters_left > 0:
                ...
                if iter_index % self.cfg.SHOW_STEP == 0:
                    pbar_str = " Epoch: {} || Batch:{:>3d}/{}|| lr: {} || " \
                               "Batch_Loss:{:>5.3f}".format(epoch, iter_index,
                                                            iter_num,
    ...
    def construct_weight_per_class(self, cls_num_list, beta):
    ...
    def skewKD_LT_KD_train(self, model_dir, label_weight, model_per_class_weights):
        optimizer = self.build_optimize(model=self.stu_model,
                                        base_lr=self.cfg.model.TRAIN.OPTIMIZER.BASE_LR,
                                        optimizer_type=self.cfg.model.TRAIN.OPTIMIZER.TYPE,
                                        momentum=self.cfg.model.TRAIN.OPTIMIZER.MOMENTUM,
        ...
        for epoch in range(1, self.cfg.model.TRAIN.MAX_EPOCH + 1):
            ...
            while iters_left > 0:
                ...
                if self.cfg.model.use_mixup:
                    ...
                    if "BSCE+MBKD" == self.cfg.approach:
                        distill_loss = BKD(pred=mixup_output_logits, soft=pre_model_output_for_distill,
                                           per_cls_weights=model_per_class_weights, T=self.cfg.model.TRAIN.out_KD_temp)
                        loss = cls_loss + distill_loss
                    elif "BSCE+O2MIM-VKD" == self.cfg.approach:
                        mixup_output_logits_for_distill = mixup_output_logits
                        if self.cfg.model.use_skewKD:
                    ...
                    if "CE+VKD" == self.cfg.approach:
                        output = self.stu_model(x)
                        output_logits = output["logits"]
                        cls_loss = criterion(output_logits, y)
                        pre_model_output_for_original_imgs = self.teacher_model(x, is_nograd=True,
                                                                                get_classifier=True)  # 获取classifier_output
                        distill_loss = self.compute_distill_loss(output_logits, pre_model_output_for_original_imgs,
                                                                 temp=self.cfg.model.TRAIN.out_KD_temp)
    ...
    def skewKD_LT_KD_train_oversample(self, model_dir, label_weight,
            ...
            while iters_left > 0:
                ...
                if self.cfg.model.use_mixup:
                    ...
                    if "CE+VKD" == self.cfg.approach:
                        ...
                        distill_loss = self.compute_distill_loss(over_output_logits, pre_model_output_for_original_imgs,
                                                                 temp=self.cfg.model.TRAIN.out_KD_temp)
                        loss = cls_loss + distill_loss
                    elif "VKD" == self.cfg.approach:
                        pre_model_output_for_original_imgs = self.teacher_model(over_x, is_nograd=True,
    ...
    def LT_train_oversample(self, model_dir, label_weight,
        ...
        if self.cfg.save_model:
            ...
            torch.save({
                'state_dict': self.best_model.state_dict(),
                'acc_result': self.best_acc,
            }, os.path.join(model_dir, "best_stu_model.pth")
            )
        pass
        pass
